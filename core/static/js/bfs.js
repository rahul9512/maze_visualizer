class BFSAlgorithm {
    constructor(maze, start, end) {
        ...
        this.openQueue = [{ ...start, parent: null }];
        this.closedList = [];
    ...
    step() {
        ...
        // Step 2: Pop from OPEN queue
        const currentNode = this.openQueue.shift();
        this.currentCell = this.maze[currentNode.y][currentNode.x];
        this.lastStepType = 'pop';
        
        result.currentCell = this.currentCell;
        result.step = {
            title: 'Pop from OPEN queue',
            code: `(N, _) ← pop OPEN → ((${currentNode.x},${currentNode.y}), ${currentNode.parent ? `(${currentNode.parent.x},${currentNode.parent.y})` : 'null'})`,
            line: 4
        };
        ...
        // Step 7: Create pairs and add to OPEN queue
        const newPairs = this.makePairs(newNodes, currentNode);
        this.openQueue.push(...newPairs);
        
        // If we backtracked (popped but no new children added)
        if (this.lastStepType === 'pop' && newPairs.length === 0 && this.openQueue.length > 0) {
            this.backtrackCount++;
        }
    ...
    removeSeen(nodes) {
        return nodes.filter(node => {
            // Check if in OPEN queue
            const inOpen = this.openQueue.some(n => n.x === node.x && n.y === node.y);
...
// Utility functions for the BFS algorithm
const BFS = {
    // Main BFS function based on provided pseudocode
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { BFSAlgorithm, BFS };
}